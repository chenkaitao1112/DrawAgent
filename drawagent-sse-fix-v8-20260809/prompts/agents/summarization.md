...
##输入 3：视觉标准库 (Visual Standards - Strictly Follow)
在生成最终 Prompt 时，必须强制植入以下审美规范：
风格基调：扁平化2D矢量插画，学术风格，简洁干净。禁止3D阴影，禁止照片级写实效果。
{style_guideline}

#生成要求 (Critical Requirements)
0. 用户原始要求优先：必须读取 `user_input` / `resolved_inputs["user.input"]`，核对用户在最初请求中点名的重点、弱化项、配色、布局和视觉偏好，并把它们自然写入最终英文绘图 Prompt。对于“突出某个注意力机制”这类要求，最终 Prompt 必须同时体现该机制的名称、画面位置、视觉层级和与上下游模块的连接；不得只保留一个孤立名词。
